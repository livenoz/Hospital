...
@Component({
  selector: 'app-treatment-detail-add',
  templateUrl: './treatment-detail-add.component.html',
  styleUrls: ['./treatment-detail-add.component.scss']
})
export class TreatmentDetailAddComponent implements OnInit {
  myForm: FormGroup;
  submitted = false;
  treatmentDetailControl = 'Thêm mới điều trị chi tiết';
  ...
  private dropdownConfig(callback) {
    this.treatmentService.getDetail(this.treatmentRouteId)
      .subscribe((treatment: TreatmentModel) => {
        this.inputData.patientId = treatment.patientId;
        this.inputData.medicalRecordId = treatment.medicalRecordId;
        this.inputData.treatmentId = treatment.id;
  ...
  private onAdd(): void {
    this.treatmentDetailService.add(this.inputData).subscribe(
      (data: number) => {
        this.spinner.hide();
        if (data > 0) {
